  ...
  (let ((s '()))
    ...
    (define (dispatch message)
      (cond ((eq? message 'push) push)
	    ...
	    (else 
	     (error "Unknown erequest -- STACK" message))))
    dispatch))
...
(define (make-preform inst machine labels operation pc)
  (let ((action (perform-action inst)))
    (if (operation-exp? action)
	(let ((action-proc (make-operation-exp action machine labels operatios)))
	  (lambda ()
	    (action-proc)
	    (advance-pc pc)))
